...
class MemoryMonitor {
  ...
  private getMemoryStats(): Promise<MemoryStats> {
  ...
  private checkModelCompatibility(memory: MemoryStats): void {
    Object.entries(this.modelRequirements).forEach(([modelId, requirements]) => {
      ...
      if (memory.available_mb < minimum) {
        this.createAlert('critical', `Insufficient memory for ${modelId}: ${memory.available_mb}MB available, ${minimum}MB minimum required`, modelId)
      } else if (memory.available_mb < required) {
        this.createAlert('warning', `Low memory for ${modelId}: ${memory.available_mb}MB available, ${required}MB required`, modelId)
      } else if (memory.available_mb < recommended) {
        this.createAlert('info', `Suboptimal memory for ${modelId}: ${memory.available_mb}MB available, ${recommended}MB recommended`, modelId)
      }
    })
  }
  ...
  private getCurrentMemory(): number | null {
    try {
      const output = spawn('free', ['-m'], { encoding: 'utf-8' })
      let result = ''
      ...
      return null // Placeholder - would need sync implementation
    } catch {
      return null
    }
  }

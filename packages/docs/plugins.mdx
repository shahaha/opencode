...
For examples, check out the [plugins](/ecosystem#plugins) created by the community.
...
```js .opencode/plugin/notification.js
...
```javascript .opencode/plugin/env-protection.js
...
```ts .opencode/plugin/custom-tools.ts
...
```ts .opencode/plugin/compaction.ts
import type { Plugin } from "@opencode-ai/plugin"

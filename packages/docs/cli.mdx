...
The OpenCode CLI by default starts the [TUI](/tui) when run without any arguments.
...
This sets up the necessary GitHub Actions workflow and guides you through the configuration process. [Learn more](/github).
...
This is useful for figuring out the exact model name to use in [your config](/config/).
...
Start a headless opencode server for API access. Check out the [server docs](/server) for the full HTTP interface.

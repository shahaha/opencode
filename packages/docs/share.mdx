...
To explicitly set manual mode in your [config file](/config):
...
You can enable automatic sharing for all new conversations by setting the `share` option to `"auto"` in your [config file](/config):
...
You can disable sharing entirely by setting the `share` option to `"disabled"` in your [config file](/config):
...
[Learn more](/enterprise) about using opencode in your organization.

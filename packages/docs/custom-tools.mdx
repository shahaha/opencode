...
Custom tools are functions you create that the LLM can call during conversations. They work alongside opencode's [built-in tools](/tools) like `read`, `write`, and `bash`.
...
```ts .opencode/tool/database.ts highlight={1}
...
```ts .opencode/tool/math.ts expandable
import { tool } from "@opencode-ai/plugin"
...
```ts .opencode/tool/project.ts highlight={8}
import { tool } from "@opencode-ai/plugin"
...
```ts .opencode/tool/python-add.ts highlight={10}
import { tool } from "@opencode-ai/plugin"

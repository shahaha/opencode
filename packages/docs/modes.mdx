...
`mode` option is now deprecated. [Learn more](/agents).
</Danger>
...
See also: [Formatters](/formatters) for information about code formatting configuration.
...
```markdown ~/.config/opencode/mode/review.md expandable
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
```json opencode.json highlight={4-14}
{
  "$schema": "https://opencode.ai/config.json",
  "mode": {
    "docs": {
...
```markdown .opencode/mode/debug.md expandable
---
temperature: 0.1
tools:
  bash: true
...
```markdown ~/.config/opencode/mode/refactor.md expandable
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:

...
```json opencode.json highlight={5}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "bash": {
...
overrides the global config. [Learn more](/agents#permissions) about agent permissions.

```json opencode.json highlight={3-7,10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "bash": {
...
```markdown ~/.config/opencode/agent/review.md
---

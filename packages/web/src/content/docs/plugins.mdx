...
Plugin commands let plugins trigger functionality directly from the prompt (`/command`), which is useful for managing plugin state or running server-side actions without prompting the LLM.

```ts title=".opencode/plugins/toggle.ts"
import type { Plugin } from "@opencode-ai/plugin"
...
- Visible commands in the slash menu
- A clean entry point for plugin-managed state
- Deterministic outputs without LLM variability

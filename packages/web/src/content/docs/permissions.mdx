...
For more complex pattern matching, you can use regular expressions by wrapping the pattern in forward slashes (`/pattern/`):
...
```json title="opencode.json"
{
  ...
  "permission": {
    "bash": {
      "*": "ask",
      "/brew (bundle|info|leaves|list|shellenv|uses)/": "allow",
      "/git\\s+(status|log|diff)/": "allow"
    }
  }
}
```

Regex patterns are evaluated as JavaScript regular expressions.

Note: Regex patterns are not anchored by default, so `/(foo|bar)/` will match anywhere in the string. Use `^` and `$` for exact matches if needed.

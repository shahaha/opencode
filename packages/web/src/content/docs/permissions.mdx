...
This lets you configure granular controls for the `read`, `edit`, `bash`, `skill`, `webfetch`, `doom_loop`, and `external_directory` tools.
...
Currently, the permissions for the `read`, `edit`, `bash`, `skill`, `webfetch`, `doom_loop`, and `external_directory` tools can be configured through the `permission` option.

---

### read

Use the `permission.read` key to control whether file reading operations require user approval.
...
Or, you can target specific file patterns and set them to `allow`, `ask`, or `deny`.
...
:::tip
When you select "accept always" for a file read, it will auto-approve future reads for files with the same extension. For example, accepting `src/index.ts` will auto-approve all `*.ts` files.
:::
...
:::tip
When you select "accept always" for a file edit, it will auto-approve future edits for files with the same extension. For example, accepting `src/index.ts` will auto-approve all `*.ts` files.
:::

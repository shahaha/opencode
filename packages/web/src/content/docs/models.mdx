...
### Inherit metadata from models.dev

When defining custom models, you can use `use_models_dev` to inherit metadata from a model on models.dev.

This is useful when you want OpenCode to reuse model metadata like token limits, pricing, capabilities, modalities, and variants while still customizing a few fields.  Its commonly used in cases where a custom proxy is in use.

```json title="opencode.json" {8-13}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "myprovider": {
      "models": {
        "my-fast-model": {
          "use_models_dev": "openrouter/anthropic/claude-sonnet-4-20250514",
        }
      }
    }
  }
...
Merge order for model metadata:

1. Explicit fields in your model config (highest priority)
2. Metadata imported from `use_models_dev`
3. OpenCode defaults

If `use_models_dev` points to an invalid model ID, OpenCode logs a warning and continues using your manual model configuration.

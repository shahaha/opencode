...
### Setting options from API documentation

Many providers display their API reference with request body parameters or an example containing a curl request.

For example, an API reference might show an example like this:

```bash frame="none" {7-10}
...
To set these options in OpenCode, add them to the `options` object:

```jsonc title="opencode.jsonc" {8-11}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "custom-provider": {
...
### OpenAI Flex Processing example

[OpenAI Flex Processing](https://platform.openai.com/docs/guides/flex-processing?api-mode=chat&lang=curl) shows you can use the flex service tier with this curl request:
...
To set 'service_tier' to 'flex' for the `gpt-5` model in OpenCode, you would add it like this:

```jsonc title="opencode.jsonc" {8}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "openai": {

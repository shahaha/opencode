...
1. **Personal Access Token (PAT)**: Set `DATABRICKS_TOKEN` environment variable or use `/connect` command
2. **OAuth M2M**: Set `DATABRICKS_CLIENT_ID` and `DATABRICKS_CLIENT_SECRET` environment variables
3. **Azure AD Service Principal**: Set `ARM_CLIENT_ID`, `ARM_CLIENT_SECRET`, and `ARM_TENANT_ID` environment variables
4. **Azure CLI**: Automatically used for Azure Databricks workspaces when logged in with `az login`

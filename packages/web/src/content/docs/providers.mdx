...
[Morpheus AI](https://mor.org) provides access to enterprise-grade AI models through a unified OpenAI-compatible API gateway, including GLM, Qwen, Kimi, Hermes, Llama, Mistral, and other state-of-the-art models.

1. Head over to the [Morpheus AI portal](https://app.mor.org), create an account, and generate an API key.

2. Run the `/connect` command and search for **Morpheus AI**.

   ```txt
   /connect
   ```

3. Enter your Morpheus AI API key.
...
4. Run the `/models` command to select a model like _GLM 4.6_, _Kimi K2 Thinking_, or _Qwen3 Coder 480B_.

   ```txt
   /models
   ```

 You can also configure Morpheus AI with additional models through your opencode config with full model specifications:
  ...
  "provider": {
    "morpheus-ai": {
      "models": {
        ...
        "hermes-3-llama-3.1-405b-instruct": {
          "reasoning": false,
          "tool_call": true,
          "attachment": false,
          "temperature": true,
...
> **Note**: All Morpheus AI models are free to use. Specifications sourced from official model providers (Z.AI, Moonshot AI, Alibaba Qwen, Meta, Mistral AI, Nous Research). Context limits and capabilities reflect base models.

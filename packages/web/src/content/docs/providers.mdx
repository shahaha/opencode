...
:::tip
When using Databricks CLI authentication, OpenCode automatically refreshes expired tokens from the CLI token cache (`~/.databricks/token-cache.json`). If your token expires, simply run `databricks auth login --profile <profile>` to refresh it.
:::

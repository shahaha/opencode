...
2. **Configure authentication** using one of the following methods:
   ...
   # Option 4: Using AWS shared credentials file (default profile)
   AWS_SHARED_CREDENTIALS_FILE=XXX opencode
   ```
   ...
   **Available options:**
   ...
   - **`AWS_PROFILE`**: Use named profiles from `AWS_SHARED_CREDENTIALS_FILE` (default: `~/.aws/credentials`). First configure with `aws configure --profile my-profile` or `aws sso login`

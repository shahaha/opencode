...
1. Head over to the [Portkey dashboard](https://app.portkey.ai/api-keys), click **Create API Key** with completions.create permission, and copy the key.

2. Configure portkey in your opencode config.
```json title="~/.config/opencode/config.json"
{
   "$schema": "https://opencode.ai/config.json",
   "provider": {
     "portkey": {
...
:::note
You can also use "npm": "@ai-sdk/openai" if you'd like to use the /responses api instead of the chat/completions api.
:::

3. Run the `/connect` command and search for Portkey.

   ```txt
   /connect
   ```

4. Enter the API key for portkey.
...
4. You can add further models in the opencode config. (checkout https://github.com/Portkey-AI/models for all available models)

5. You can specify fallbacks, rate limits and more in the portkey config so your requests never fail! (https://portkey.ai/docs/product/ai-gateway/configs)

...
- Global Claude-compatible: `~/.claude/commands/`
- Per-project Claude-compatible: `.claude/commands/`

:::note
OpenCode native paths (`.opencode/command/`) take precedence over Claude-compatible paths (`.claude/commands/`).
This allows seamless migration from Claude Code while still supporting OpenCode-specific overrides.
:::

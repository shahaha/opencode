...
OpenCode first attempts to use `osascript`, then falls back to `pbcopy`/`pbpaste`. If neither native command is available, it uses its built-in clipboard helper as a last resort.

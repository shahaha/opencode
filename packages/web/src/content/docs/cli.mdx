...
| `OPENCODE_ENABLE_OPENTELEMETRY`       | boolean | Enable OpenTelemetry spans for AI SDK calls       |
...
| `OPENCODE_EXPERIMENTAL_MCP_TIMEOUT_MS`          | number  | Timeout in ms for MCP requests          |
...
| `OPENCODE_EXPERIMENTAL_DISABLE_PASTE_SUMMARY`   | boolean | Disable paste summary in TUI            |
| `OPENCODE_EXPERIMENTAL_BATCH_TOOL`              | boolean | Enable batch tool                       |
| `OPENCODE_EXPERIMENTAL_CONTINUE_LOOP_ON_DENY`   | boolean | Continue loop when tool call is denied  |

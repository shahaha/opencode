...
Reload the current instance. This re-reads your config (including `opencode.json` and `.opencode/opencode.jsonc`) without restarting the TUI.

```bash frame="none"
/reload
```

_Aliases_: `/restart`, `/refresh`

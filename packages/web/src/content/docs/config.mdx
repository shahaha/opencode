...
Enable OpenTelemetry tracing for monitoring and debugging. This exports traces and structured logs to an OTLP-compatible collector like [Aspire Dashboard](https://learn.microsoft.com/en-us/dotnet/aspire/fundamentals/dashboard/overview).
...
Set the endpoint using the standard `OTEL_EXPORTER_OTLP_ENDPOINT` environment variable (defaults to `http://localhost:4317`).

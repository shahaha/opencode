...
Enable durable streaming architecture with event persistence and server discovery.
...
- **Event Persistence**: All session events are persisted to SQLite with ULID offsets
- **Catch-up Streaming**: Clients can resume streams from any offset
- **Server Discovery**: Multi-server registry with automatic heartbeat
- **SSE Streaming**: Live server-sent events with catch-up support

This enables building resilient clients that can reconnect and resume without losing events.

**Default:** `false` (opt-in)

**Note:** This is an experimental feature under active development. The API may change in future releases.

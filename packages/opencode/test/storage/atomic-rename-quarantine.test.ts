...
describe("storage.atomicWrite quarantine on rename failure", () => {
  test("moves tmp to quarantine when target path is a directory", async () => {
    const dir = storageDir()
    const key = ["test", "atomic", "conflict"]
    const target = path.join(dir, ...key) + ".json"

    await fs.mkdir(path.dirname(target), { recursive: true })
    await fs.mkdir(target, { recursive: true })

    await expect(Storage.write(key, { ok: true } as any)).rejects.toThrow()

#!/bin/bash
# OpenCode AG-UI Docker 環境修復腳本

echo "🔧 修復 Docker 環境配置問題"
echo "============================"
...
# 檢查端口衝突
echo -e "\n📋 檢查端口使用情況:"
echo "端口 80 (JupyterHub): $(lsof -i :80 2>/dev/null | wc -l) 個進程"
echo "端口 8080 (OpenCode 前端): $(lsof -i :8080 2>/dev/null | wc -l) 個進程"
echo "端口 3001 (OpenCode 後端): $(lsof -i :3001 2>/dev/null | wc -l) 個進程"

# 創建修復後的啟動腳本
cat > start-lan-fixed.sh << 'EOF'
#!/bin/bash
